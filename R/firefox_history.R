...
read_firefox_history <- function(path_to_firefox_history_db) {
  ...
  history = DBI::dbReadTable(con, "moz_historyvisits") |>
    as_tibble()

  places = DBI::dbReadTable(con, "moz_places") |>
    as_tibble()

  df = tidyr::left_join(history, places, by = c("place_id" = "id")) |>
    select(place_id, visit_date, url, title, origin_id, visit_count) |>
    # UNIX timestamp is in microseconds
    mutate(
      visit_date = as.POSIXct(visit_date / 1e6, origin = "1970-01-01"),
