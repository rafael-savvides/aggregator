...
read_work_diary_hours <- function(path_to_work_diary, year = 2020) {
  ...
  work_hours = work_diary |>
    filter(
      !stringr::str_detect(X, "^\\d\\d?:\\d\\d"),
      !stringr::str_detect(X, "^Extra hours"),
      X != ""
    ) |>
    pivot_wider(date, X) |>
    janitor::clean_names()
...
read_work_diary_tasks <- function(path_to_work_diary, year = 2020) {
  ...
  work_tasks = work_diary |>
    filter(stringr::str_detect(X, "^\\d\\d?:\\d\\d") | stringr::str_detect(X, "^Extra hours")) |>
    rename(time = X) |>
...
read_recordings_notes <- function(path_to_recording_notes, sheet = 1) {
  ...
  readxl::read_excel(path_to_recording_notes, sheet = sheet, col_types = col_types)
}
