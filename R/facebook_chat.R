...
read_facebook_chat <- function(path_to_fb_dir) {
  parse_messages = function(messages) {
    ...
    if ("call_duration" %in% colnames(m)) {
      m = m |>
        mutate(
          content = if_else(
            !is.na(call_duration) & call_duration != 0,
            stringr::str_c(content, " ", call_duration, "sec"),
            content
          )
        )
    }
    ...
    m |>
      mutate(
        has_photos = "photos" %in% names(messages) && !map_lgl(photos, is.null),
  ...
  fb = tibble(
    chat = sapply(fb_chats, function(x) x$title),
