...
read_bank <- function(path_to_bank_dir) {
  danske = list.files(path_to_bank_dir, "danske.+\\.csv", full.names = TRUE) |>
    purrr::map_df(read_danske) |>
    mutate(account = "Danske")
  spankki = list.files(path_to_bank_dir, "spankki.+\\.csv", full.names = TRUE) |>
    purrr::map_df(read_spankki) |>
    mutate(account = "S-Bank")
  spankki2 = list.files(path_to_bank_dir, "spankki_s.+\\.csv", full.names = TRUE) |>
    purrr::map_df(read_spankki) |>
...
read_spankki <- function(path_to_csv) {
  ...
  book <- read.csv(
    ...
    encoding = "UTF-8",
    col.names = c(
      "date_reported", "date_paid", "amount", "type", "payer", "receiver", "receiver_iban", "receiver_bic",
  ...
  book |>
    mutate(
      amount = make_numeric(amount),
      date_reported = day_month_year2date(date_reported),
      date = day_month_year2date(date_paid)
    ) |>
    select(date, amount, receiver, payer, message, type, receiver_iban)
}
...
read_danske <- function(path_to_csv) {
  ...
  read.csv2(path_to_csv,
    ...
    mutate(
      payer = if_else(amount > 0, as.character(receiver_payer), "Rafael Savvides"),
    ...
    mutate(receiver = stringr::str_replace(receiver, " +\\)\\)\\)\\)", ""))
}
