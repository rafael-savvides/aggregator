...
read_msn <- function(path_to_msn_dir) {
  parse_chat_html <- function(chat_html) {
    parse_chat_text <- function(chat_text) {
      chat_text |>
        mutate(
          time = stringr::str_extract(X1, "\\(.+\\)"),
          time = stringr::str_remove_all(time, "\\(|\\)"),
          sender = stringr::str_remove_all(X1, "\\(.+\\) ?|:$"),
          message = X2
        ) |>
        select(-X1, -X2)
    }
    session_start = rvest::html_text(rvest::html_nodes(chat_html, xpath = "/html/body/div/h2"))
    ...
    participants = lapply(
      1:n_sessions,
      function(i) rvest::html_text(rvest::html_nodes(chat_html, xpath = paste0("/html/body/div[", i, "]/ul/li")))
    )
    chat_text = sapply(
      1:n_sessions,
      function(i) rvest::html_table(rvest::html_nodes(chat_html, xpath = paste0("/html/body/div[", i, "]/table")))
    )
    df = data.frame(
      ...
      mutate(chat_text = I(purrr::map(chat_text, parse_chat_text)))
    df
  }

  html_files = data.frame(
    full_path = list.files(path_to_msn_dir, "*.html",
  ...
  for (i in 1:n) {
    ...
    chat_html = rvest::read_html(html_files$full_path[i])
    list_of_chats[[i]] = parse_chat_html(chat_html)
  }

  msn = data.frame(
    account = html_files$account[1:n],
    chat = I(list_of_chats),
    stringsAsFactors = FALSE
  ) |>
    unnest(chat) |>
    mutate(
      participants = purrr::map_chr(participants, paste0, collapse = "|"),
      session_start = stringr::str_remove_all(session_start, "^Session Start: |,"),
      has_weekday_name = stringr::str_detect(session_start, "^[a-zA-Z]+ "),
