...
read_telegram <- function(dir_telegram) {
  json = jsonlite::read_json(dir_telegram)
  chats = json$chats$list
  df = tibble(
    chat = sapply(chats, function(x) {
  ...
  parse_chat = function(x) {
    ...
    x_df = tibble(chat = x) |>
      ...
      select(-intersect(
        colnames(x_df),
        c(
          "type", "from_id", "file", "thumbnail", "sticker_emoji",
    ...
    if ("mime_type" %in% colnames(x_df)) {
      x_df = x_df |>
        mutate(
          text = ifelse(
            mime_type == "", # may need to be mutate(content = ...)
            text,
            paste0(
              ifelse(text == "", text, paste0(text, " \n")),
              "Media: ",
              mime_type
            )
          )
